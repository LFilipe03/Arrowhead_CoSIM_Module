...
void CaService::indicate(const vanetza::btp::DataIndication& ind, std::unique_ptr<vanetza::UpPacket> packet)
{
	...
	if (cam && cam->validate()) {
		...
		EV << "SIGNAL CAM RECEIVED EMITTED!" << endl;
...
void CaService::sendCam(const SimTime& T_now)
{
	...
	EV << "SIGNAL CAM SENT EMITTED!" << endl;
...
vanetza::asn1::Cam createCooperativeAwarenessMessage(const VehicleDataProvider& vdp, uint16_t genDeltaTime)
{
	...
	// Access the occupancygrid_data array
    int8_t* occupancygrid_data = (*message).occupancygrid_data;
    int& timestamp_nanosec = (*message).timestamp_nanosec;
    int& timestamp_sec = (*message).timestamp_sec;

    // Access the data from the occupancygrid message
    const auto& data_array = vdp.getOccupancyGrid().data;
    // Get the stamp in nanosec
    const auto& timenano = vdp.getOccupancyGrid().header.stamp.nanosec;
    
    // Get the stamp in sec
    const auto& timesec= vdp.getOccupancyGrid().header.stamp.sec;
    
    timestamp_nanosec = timenano;
    timestamp_sec = timesec;

    // Copy the data_array to the occupancygrid_data array
    std::copy(data_array.begin(), data_array.end(), occupancygrid_data);
