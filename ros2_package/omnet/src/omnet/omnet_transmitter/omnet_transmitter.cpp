#include "omnet/omnet_transmitter.hpp"
...
OmnetTransmitter::OmnetTransmitter(std::string robot_name):Node("omnet_transmitter_node")
{
    ...
    if(robot_name.find("central") != std::string::npos) {
    	ocgrid_subscriber = this->create_subscription<nav_msgs::msg::OccupancyGrid>("global_costmap/costmap", 2, [this](const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
    	{ocgrid_callback(msg);});
    }
...
// Receive and save the occupancygrid data
void OmnetTransmitter::ocgrid_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
    this->occupancygrid_ = *msg;
}
...
void OmnetTransmitter::cam_callback()
{
    ...
    msg.occupancygrid = this->occupancygrid_;
    //msg.occupancygrid.header.stamp = this->get_clock()->now();
    publisher->publish(msg);
    //const auto current_time = this->get_clock()->now();
    //RCLCPP_INFO(this->get_logger(), "Current time: %f seconds, %ld nanoseconds", current_time.seconds(), current_time.nanoseconds());
}
