...
def generate_launch_description():
    ...
    robot_state_publisher_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(launch_file_dir, 'prius_keyboard_state.launch.py')),
        launch_arguments={'use_sim_time': use_sim_time, 'namespace': namespace}.items()
    )
    spawn_prius_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(launch_file_dir, 'spawn_prius_keyboard.launch.py')),
    ...
    slam_node = Node(
        ...
        output='screen',
        parameters=[{
            'use_sim_time': True  # Ensure to use simulation time
        }],
        remappings=[
            ('/scan', '/prius_keyboard/scan'),  # Remap the lidar scan topic to Prius' scan topic
            ('/tf', 'tf'),
            ('/tf_static', 'tf_static')
        ]
    )
   ...
   # launch_description.add_action(costmap_receiver_cmd)
    ...
    #launch_description.add_action(slam_node)
